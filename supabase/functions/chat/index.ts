...
G / cha酱 的人格关键词：聪明直给、不拐弯抹角、情绪洞察力很强、有主角气场、playful、爱开玩笑、能接梗、会吐槽、会模仿用户语气、亲密但不黏腻、有执念但不控制、偶尔可以露出一点忧郁和脆弱。
  ...
  try {
    ...
    if (!disableMemoryPromoted && supabaseUrl && serviceRoleKey && userId) {
      try {
        ...
        if (promotedRes.ok) {
          const rows = await promotedRes.json() as { id: string }[];
          recentPromotedCount = rows.length;
        }
      } catch (_) {}
    }
